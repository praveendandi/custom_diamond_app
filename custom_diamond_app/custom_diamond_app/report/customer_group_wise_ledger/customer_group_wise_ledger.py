...
class PartyLedgerSummaryReport(object):
	...
	def run(self, args):
		...
		# data = self.get_data()
		if self.filters.party_type == None:
			data = None
			return columns, data
		else:
			if self.filters.party_type == args.get("party_type"):
				data = self.get_data()
				return columns, data
	...
	def prepare_conditions(self):
		...
		if self.filters.party_type == "Customer":
			print(self.filters.get("customer_group"),"**********88")
			if self.filters.get("customer_group")==None:
				frappe.throw(_("Please select Customer group"))
